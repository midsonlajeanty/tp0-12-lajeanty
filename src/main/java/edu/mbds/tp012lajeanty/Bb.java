package edu.mbds.tp012lajeanty;

import edu.mbds.tp012lajeanty.service.Modificateur;
...
public class Bb implements Serializable {
    ...
    /**
     * Numéro de la question courante dans la conversation (1 pour la première).
     */
    private int numeroQuestion = 0;

    /**
     * Horodatage de la question précédente, pour calculer le délai entre deux échanges.
     */
    private Instant instantPrecedente;
     ...
     * Traitement personnel (bonus) : pour chaque question, le serveur génère une
     * réponse contenant le numéro de la question, le délai écoulé depuis la
     * précédente et quelques statistiques sur le texte saisi. Le rôle système
     * est ajouté au début de la toute première réponse.
     *
    ...
    public String envoyer() {
        ...
        Duration depuisPrecedente = null;

        if (this.instantPrecedente != null) {
            depuisPrecedente = Duration.between(this.instantPrecedente, maintenant);
        }

        this.instantPrecedente = maintenant;

        this.reponse = this.modificateur.modifier(
                this.question,
                roleSystemePourModification,
                this.numeroQuestion,
                depuisPrecedente
        );
    ...
    public List<SelectItem> getRolesSysteme() {
        if (this.listeRolesSysteme == null) {
            ...
            String role = """
                    ...
                    """;
            // 1er argument : la valeur du rôle, 2ᵉ argument : le libellé du rôle
